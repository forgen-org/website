...
<Tabs items={['Rust']}>
<Tab>
```rust filename="application/ports.rs"
use framework::*;
pub use domain::{BankAccount, BankAccountEvent}

pub trait BankAccountStore {
    async fn get_account(&self, name: &str) -> AnyResult<BankAccount>;
    async fn push_account(&self, name: &str, events: &[BankAccountEvent]) -> AnyResult<()>;
}
```
</Tab>
</Tabs>
