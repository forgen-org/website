...
const withdraw = (account: Account, amount: number): AccountEvent[] | string => {
  if (action === 'withdraw' && account.balance < amount) return 'Insufficient funds';
  return [{ type: 'Withdrawn', amount }];
};

const deposit = (account: Account, action: amount: number): AccountEvent[] | string => {
  return [{ type: 'Deposited', amount }];
};
